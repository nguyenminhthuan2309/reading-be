  ...
  return (
    <ThemeProvider theme={theme}>
      <Box sx={{ display: "flex", width: "100%", height: "100%" }}>
            ...
            {tabs.map((tab) => (
              <Box key={tab.id}>
                <ListItem disablePadding>
                  <ListItemButton
                    selected={activeTab === tab.id && (!tab.subTabs || !activeSubTab)}
                    onClick={() => (tab.subTabs ? toggleExpand(tab.id) : handleTabClick(tab.id, false))}
                    sx={{
                      py: 2,
                      "&.Mui-selected": {
                  ...
                  >
                    <ListItemIcon sx={{ color: "white", minWidth: 40 }}>{tab.icon}</ListItemIcon>
                    <ListItemText primary={tab.label} />
                    {tab.subTabs && (expandedTabs.includes(tab.id) ? <ExpandLessIcon /> : <ExpandMoreIcon />)}
                  </ListItemButton>
                </ListItem>

                {/* Subtabs */}
                {tab.subTabs && (
                  <Collapse in={expandedTabs.includes(tab.id)} timeout="auto" unmountOnExit>
                    <List component="div" disablePadding>
                      {tab.subTabs.map((subtab) => (
                        ...
                        >
                          <ListItemIcon sx={{ color: "white", minWidth: 40 }}>{subtab.icon}</ListItemIcon>
        ...
        >
          {/* Page Title */}
          <Typography variant="h5" component="h1" sx={{ p: 2, borderBottom: "1px solid", borderColor: "divider" }}>
            {getPageTitle()}
          </Typography>
