...
import { resetStateCreateBook } from "@/utils/redux/slices/bookReducer/createBook";
...
function BookBasicInfo() {
  ...
  const loadingCreateBook = useSelector((state) => state.createBook.loading);
  const { bookData } = useSelector((state) => state.createBook);
  ...
  const handleSubmitBookInfo = async (data) => {
    ...
    const res = await dispatch(createBook(formData));
    return res;
  }
  ...
  useEffect(() => {
    ...
    if (actionType === "gallery") {
      ...
      handleRedirectCreateChapter(bookData, +bookType);
    }
  }, [bookData, actionType, bookType, handleRedirectCreateChapter, router]);

  useEffect(() => {
    return () => {
      dispatch(resetStateCreateBook());
    };
  }, []);
  return (
    <section className="flex flex-wrap gap-9 self-stretch max-md:max-w-full">
      {loadingCreateBook ? (
        ...
        <form
          onSubmit={handleSubmit(handleSubmitBookInfo)}
          className="w-full"
        >
        ...
        {+bookType === 2 && (
          ...
          <div className="flex flex-wrap gap-8 justify-self-center mt-24 text-xl text-white w-[907px] max-md:mt-10">
            <button
              type="submit"
              disabled={loadingCreateBook || loading}
              onClick={() => setActionType("gallery")}
