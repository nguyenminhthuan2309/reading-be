...
export class BanIpMiddleware implements NestMiddleware {
    ...
    async use(req: Request, res: Response, next: NextFunction) {
        ...
        if (!ip) {
            return res.status(400).json({ status: false, code: 'INVALID_IP', message: 'Cannot detect IP address' });
        }
        ...
        if (isBanned) {
            return res.status(403).json({ status: false, code: 'IP_BANNED', message: 'Your IP is temporarily blocked' });
        }
        ...
        if (reqCount >= this.MAX_ATTEMPTS) {
            ...
            return res.status(429).json({ status: false, code: 'TOO_MANY_REQUESTS', message: 'Too many requests. You are temporarily blocked' });
        }
